...
data {
  ...
  // covariate data
  array[n_strata,n_years] real cov; //covariate data by strata
...
// second half of time-series - runs forwards from fixed_year
  ...
  for(i in 1:n_train){
    ...
    E[i] =  strata[strat_tr[i]] + yeareffect[strat_tr[i],year_tr[i]] + (beta_cov[strat_tr[i]]*cov[strat_tr[i],year_tr[i]]) + eta*first_year_tr[i] + ste + obs + noise;
...
for(y in 1:n_years){
  ...
  real obs = sdobs*obs_raw[obs_mat[s,t]]; // observer intercepts
      ...
      n_t[t] = exp(strata[s] + yeareffect[s,y] + (beta_cov[s]*cov[s,y]) + retrans_noise + ste + obs);
      }
